...
export default function PersistentDrawerLeft() {
  ...
  function componentSelector() {
    if (visible === 0)
      ...
      return <CategoryList texto={"Listagem de categoria"} />;
  }
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      >
        ...
        <Divider />
        <List>
          {['Cadastro de Produtos', 'Listagem de Produtos', 'Cadastro de Categorias', 'Listagem de Categorias'].map((text, index) => (
            <ListItem onClick={() => { setVisible(index) }} key={index} disablePadding>
              <ListItemButton>
                <ListItemIcon>
                  {iconSelector(index)}
                </ListItemIcon>
